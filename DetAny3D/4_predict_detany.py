...
checkpoint = torch.load(cfg.resume, map_location=f'cuda:'+str(args.device))
...
my_sam_model.to('cuda:'+str(args.device))
my_sam_model.setup()
my_sam_model.eval()
...
for json_path3 in tqdm(dirs[start:end]):
    ...
    try:
        ...
        with open(json_path3, "r") as f:
            json_data_s3 = json.load(f)

        json_data_s4 = copy.deepcopy(json_data_s3)

        for ind, frame in enumerate(json_data_s3['frames']):
            ...
            with torch.no_grad():
                ...
                bbox_2d_list = [] # bbox list 
                label_list = [] # label unknow
                point_coords_list = []
                ...
                for det_id, detection in enumerate(frame['detections']):
                    ...
                    bbox_2d = (o1_x1, o1_y1, o1_x2, o1_y2)
                ...
                img_for_sam = preprocess(img, cfg).to('cuda:'+str(args.device))
                img_for_dino = preprocess_dino(img).to('cuda:'+str(args.device))
                ...
                if mode == 'box':
                    ...
                    input_dict = {
                        ...
                        "images_shape": torch.Tensor(before_pad_size).to('cuda:'+str(args.device)).unsqueeze(0),
                        "image_for_dino": img_for_dino,
                        "boxes_coords": bbox_2d_tensor,
                    }
                if mode == 'point':
        
                    points_2d_tensor = torch.stack(point_coords_list, dim=1).to('cuda:'+str(args.device))
                ...
                K = ret_dict['pred_K']
                json_data_s4['frames'][ind]['detany_intrinsics'] = K.tolist()
                ...
                for i in range(len(decoded_bboxes_pred_2d)):
                    ...
                    json_data_s4['frames'][ind]['detections'][i]['detany_3D']      = [float(x),float(y),float(z)]
                    json_data_s4['frames'][ind]['detections'][i]['detany_whlyaw']  = [float(w),float(h),float(l),float(yaw)]
                    json_data_s4['frames'][ind]['detections'][i]['detany_6Dpose']  = rot_mat_i.tolist()
                    # print draw vertices_2d
        ...
        p2 = Path(json_path3)    
        new_path2 = Path(str(p2).replace("jsons_step3", "jsons_step4"))
